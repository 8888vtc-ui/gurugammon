...
export interface GameState {
  ...
  whitePlayer: any;
  blackPlayer: any;
  boardState: number[]; // [white1, black1, white2, black2, ...]
...
export class BackgammonEngine {
   ...
   */
  static getWinner(gameState: GameState): 'WHITE' | 'BLACK' | null {
    const whiteTotal = gameState.boardState
  ...
  // Helper methods
  private static hasPiecesOnBar(gameState: GameState, player: 'WHITE' | 'BLACK'): boolean {
    const barIndex = player === 'WHITE' ? 0 : 1;
    return (gameState.boardState[barIndex] || 0) > 0;
  }

  private static getPiecesOnPoint(gameState: GameState, point: number, player: 'WHITE' | 'BLACK'): number {
    if (point === 0) { // Bar
  ...
  private static getOpponentPiecesOnPoint(gameState: GameState, point: number, player: 'WHITE' | 'BLACK'): number {
    const opponent = player === 'WHITE' ? 'BLACK' : 'WHITE';
    return this.getPiecesOnPoint(gameState, point, opponent);
  }

  private static isBearingOffMove(from: number, to: number, player: 'WHITE' | 'BLACK'): boolean {
    if (player === 'WHITE') {
  ...
  private static canBearOff(gameState: GameState, player: 'WHITE' | 'BLACK'): boolean {
  ...
  private static isValidBearingOff(gameState: GameState, from: number, to: number, player: 'WHITE' | 'BLACK'): boolean {
    const diceValue = Math.abs(to - from);
  ...
  private static isValidMoveDirection(from: number, to: number, player: 'WHITE' | 'BLACK'): boolean {
    if (player === 'WHITE') {
  ...
  static getInitialBoardState(): number[] {
    const board = new Array(48).fill(0); // 24 points × 2 players
